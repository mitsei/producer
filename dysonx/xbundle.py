...
"""
DESCRIPTOR_TAGS = ['course','chapter','sequential','vertical', 'split_test','html','problem','video', 'videoalpha',
...
class XBundle(object):
    ...
    def set_course(self,xml):
        ...
        if 'semester' in xml.attrib:
            semester = xml.get('semester', self.DefaultSemester)
        # semester = xml.get('url_name', xml.get('semester', self.DefaultSemester))
        else:
            semester = xml.get('url_name', self.DefaultSemester)
...
#    This is the original, unadulterated version that works just fine for un-Python-unzipped courses
...
#        # print self.pp_xml(self.course)
    ...
    def import_course_from_directory(self, dir):
        ...
        try:
            ...
            with open(dir / 'course.xml') as coursefile:
                root = dir + coursefile.read()
            x = etree.parse(root).getroot()
        if 'semester' in x.attrib:
