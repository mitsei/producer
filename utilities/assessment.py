...
from dlkit.mongo.osid.osid_errors import InvalidArgument, NullArgument,\
    Unsupported, NotFound
from dlkit.mongo.primitives import DataInputStream
from dlkit.mongo.records.types import ANSWER_GENUS_TYPES, ANSWER_RECORD_TYPES

from dlkit_django.primordium import Id, Duration, DateTime

# Instructions for using Proxy:
# http://github.mit.edu/oeit/dlkit/issues/5#issuecomment-295
# from dlkit.services.proxy import ProxyManager as PM
# from dlkit.services.assessment import AssessmentManager as AM
...
from assessmentsv2.types import *
...
def activate_managers(request):
    ...
    """
    if 'am' not in request.session:
        condition = PROXY_SESSION.get_proxy_condition()
        condition.set_http_request(request)
        proxy = PROXY_SESSION.get_proxy(condition)
        set_session_data(request, 'am', RUNTIME.get_service_manager('ASSESSMENT', proxy=proxy))

    if 'cm' not in request.session:
        condition = PROXY_SESSION.get_proxy_condition()
        condition.set_http_request(request)
        proxy = PROXY_SESSION.get_proxy(condition)
        set_session_data(request, 'cm', RUNTIME.get_service_manager('COMMENTING', proxy=proxy))
    return request
...
def get_object_bank(manager, object_id, object_type='item', bank_id=None):
    ...
    return manager.get_bank(clean_id(bank_id))
...
def get_object_bank_from_request(request):
    ...
    if any(p in path for p in valid_params):
        test_request = TestRequest(username=request.META.get('HTTP_X_API_PROXY', ''))
        condition = PROXY_SESSION.get_proxy_condition()
        ...
        try:
            if '/items/' in path:
                ...
                object_id = path.split('/assessmentsoffered/')[-1]
    ...
    if question['type'] == 'question-record-type%3Ashort-text-answer%40ODL.MIT.EDU':
        ...
        if create:
            ...
            if not ('frontView' in files and 'sideView' in files and 'topView' in files):
                raise NullArgument('All three view set')
            else:
        ...
        if create:
            ...
            if 'manip' in files:
                ...
                if not ('choice0small' in files and 'choice0big' in files):
                    raise NullArgument('At least two choice set')
